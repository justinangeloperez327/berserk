...
fn build_app() -> Result<App> {
    ...
    app.delete("/items/{id}", |_| Response::empty().status(204))?;
    Ok(app)
}
...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    let address = std::env::args().nth(1).unwrap_or_else(|| "127.0.0.1:3000".to_owned());
...
mod tests {
    ...
    fn exchange(address: SocketAddr, method: &str, path: &str, body: &[u8]) -> Vec<u8> {
        ...
        let mut output = Vec::new(); stream.read_to_end(&mut output).unwrap(); output
    }
    ...
    fn consumer_http_contract() {
        ...
        for (method, path, input, status, expected) in [
            ...
            assert!(headers.starts_with(&format!("HTTP/1.1 {status} ")), "{method} {path}");
            assert_eq!(&bytes[split+4..], expected);
            if status == 405 { assert!(headers.contains("allow: GET, HEAD")); }
            if method == "HEAD" { assert!(headers.contains("content-length: 2")); }
            if status == 204 { assert!(!headers.contains("content-length:")); }
        }
        drop(cleanup);
