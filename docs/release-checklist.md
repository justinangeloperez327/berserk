...
- [ ] Generate checksums and provenance/signing material according to the chosen release platform. `.github/workflows/release-artifacts.yml` now records a deterministic source snapshot, all 15 Cargo package-file lists, `RELEASE-MANIFEST.json` with the internal dependency graph and publication order, and verified `SHA256SUMS`; manual runs add GitHub/Sigstore provenance. Final `.crate` archives are intentionally produced later during sequential crates.io publication because dependent `0.1.0` crates do not exist in the registry before the first release. Close this gate only after a manual run from the intended release ref succeeds and its checksums and attestation are verified according to `docs/release-provenance.md`.
