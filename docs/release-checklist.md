...
- [ ] Generate checksums and provenance/signing material according to the chosen release platform. `.github/workflows/release-artifacts.yml` now builds all 15 release-candidate `.crate` archives, records `RELEASE-MANIFEST.json`, generates and verifies `SHA256SUMS`, and uses GitHub/Sigstore provenance on manual release-artifact runs. Close this gate only after a manual run from the intended release ref succeeds and its checksums and attestation are verified according to `docs/release-provenance.md`.
