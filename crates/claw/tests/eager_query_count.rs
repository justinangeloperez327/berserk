...
struct CountingConnection { inner: SqliteConnection, queries: usize }
impl Connection for CountingConnection {
    ...
    fn query(&mut self, statement: &Statement) -> Result<Vec<Row>> { self.queries += 1; self.inner.query(statement) }
...
fn typed_eager_loading_is_constant_query_count_for_parent_batch() {
    ...
    assert_eq!(c.queries, 2, "one parent query plus one batched relationship query");
}
