...
use berserk_database::{
    drivers::sqlite::SqliteConnection, Capabilities, Connection, Execution, Statement, Transaction,
    TransactionOptions,
};
...
fn children() -> HasMany<Parent, Child> {
    HasMany::new("parent_id", Parent::key, |c| c.parent_id.into())
}
...
fn typed_eager_loading_is_constant_query_count_for_parent_batch() {
    ...
    inner
        .execute(&Statement::new(
            "INSERT INTO parents VALUES (1), (2), (3), (4)",
        ))
        .unwrap();
    inner
        .execute(&Statement::new(
            "INSERT INTO children VALUES (10,1),(11,1),(12,2),(13,4)",
        ))
        .unwrap();
    let mut c = CountingConnection { inner, queries: 0 };
    ...
    assert_eq!(
        c.queries, 2,
        "one parent query plus one batched relationship query"
    );
}
