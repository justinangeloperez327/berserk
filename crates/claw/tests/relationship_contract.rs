...
fn children() -> HasMany<Parent, Child> {
    HasMany::new("parent_id", Parent::key, |c| {
        c.parent_id.map(Value::from).unwrap_or(Value::Null)
    })
}
fn one_child() -> HasOne<Parent, Child> {
    HasOne::new("parent_id", Parent::key, |c| {
        c.parent_id.map(Value::from).unwrap_or(Value::Null)
    })
}
fn parent() -> BelongsTo<Child, Parent> {
    BelongsTo::new("id", |c| c.parent_id.map(Value::from), Parent::key)
}
...
fn relationships_batch_parents_and_preserve_cardinality() {
    ...
    .unwrap();
    c.execute(&Statement::new(
        "CREATE TABLE children (id INTEGER PRIMARY KEY, parent_id INTEGER NULL)",
    ))
    .unwrap();
    c.execute(&Statement::new("INSERT INTO parents VALUES (1), (2), (3)"))
        .unwrap();
    c.execute(&Statement::new(
        "INSERT INTO children VALUES (10, 1), (11, 1), (12, 2), (13, NULL)",
    ))
    .unwrap();
...
fn signed_and_unsigned_relationship_keys_match() {
    ...
    .unwrap();
    c.execute(&Statement::new("INSERT INTO children VALUES (10, 7)"))
        .unwrap();
