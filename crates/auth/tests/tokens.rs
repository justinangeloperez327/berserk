use berserk_auth::{
    ErrorKind, Guard, MemorySessionStore, MemoryTokenStore, Principal, SessionManager,
    TokenDigest, TokenManager, TokenRecord, TokenStore,
};
...
fn tokens_are_unique_scoped_and_reusable_until_revoked() {
    ...
    for _ in 0..64 {
        let token = tokens.issue(user(), ["users.read", "users.update"], 100).unwrap();
        ...
        for now in [100, 101, u64::MAX] {
            ...
            assert_eq!(principal.abilities().collect::<Vec<_>>(), ["users.read", "users.update"]);
        }
        tokens.revoke(&token.digest(), 102).unwrap();
        assert!(tokens.authenticate(token.expose(), 103).unwrap().is_none());
        assert_eq!(tokens.resolve(token.expose(), 103).unwrap_err().kind(), ErrorKind::RevokedToken);
...
fn expiration_is_exclusive_and_overflow_is_rejected() {
    let tokens = TokenManager::with_ttl(MemoryTokenStore::default(), Duration::from_secs(10)).unwrap();
    ...
    assert_eq!(tokens.resolve(token.expose(), 110).unwrap_err().kind(), ErrorKind::ExpiredToken);
    assert!(tokens.authenticate(token.expose(), 110).unwrap().is_none());
    assert_eq!(tokens.issue(user(), ["read"], u64::MAX).unwrap_err().kind(), ErrorKind::Configuration);
...
fn scopes_cannot_be_widened_or_interpreted_as_wildcards() {
    ...
    assert_eq!(tokens.issue(scoped.clone(), ["users.update"], 1).unwrap_err().kind(), ErrorKind::Forbidden);
    ...
    for scope in ["*", "users.*", "", "users read"] {
        assert_eq!(tokens.issue(user(), [scope], 0).unwrap_err().kind(), ErrorKind::Configuration);
    }
}
...
fn memory_capacity_pruning_and_collision_rejection_preserve_existing_tokens() {
    ...
    assert_eq!(tokens.issue(user(), ["read"], 1).unwrap_err().kind(), ErrorKind::Store);
    ...
    let expiring = TokenManager::with_ttl(MemoryTokenStore::new(1).unwrap(), Duration::from_secs(1)).unwrap();
    ...
    let record = TokenRecord { principal: user(), issued_at: 0, expires_at: None, revoked_at: Some(1) };
    store.put(digest, record.clone()).unwrap();
    let mut replacement = record.clone();
    replacement.revoked_at = None;
    assert_eq!(store.put(digest, replacement).unwrap_err().kind(), ErrorKind::Store);
...
fn session_credentials_and_api_credentials_are_separate() {
    let tokens = TokenManager::new(MemoryTokenStore::default());
    let sessions = SessionManager::new(MemorySessionStore::default(), Duration::from_secs(60)).unwrap();
...
impl TokenStore for Capture {
    ...
    fn get(&mut self, _: &TokenDigest) -> berserk_auth::Result<Option<TokenRecord>> { Ok(None) }
    fn revoke(&mut self, _: &TokenDigest, _: u64) -> berserk_auth::Result<()> { Ok(()) }
    fn remove_expired(&mut self, _: u64) -> berserk_auth::Result<u64> { Ok(0) }
}

#[test]
fn storage_receives_only_digest_and_redacted_metadata() {
    let captured = Arc::new(Mutex::new(None));
    let tokens = TokenManager::with_ttl(Capture(captured.clone()), Duration::from_secs(60)).unwrap();
    let token = tokens.issue(user().with_claim("secret", "claim-secret"), ["read"], 100).unwrap();
