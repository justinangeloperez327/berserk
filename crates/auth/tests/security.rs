...
fn malformed_and_unknown_tokens_fail_without_exposing_input() {
    ...
    let inputs = [
        ...
        "secret".to_owned(),
        "bst_".to_owned(),
        format!("bst_{}", "a".repeat(63)),
        format!("bst_{}", "a".repeat(65)),
        format!("bst_{}", "A".repeat(64)),
        format!("bst_{}", "z".repeat(64)),
...
impl TokenStore for FailingTokenStore {
    fn put(&mut self, _: TokenDigest, _: TokenRecord) -> Result<()> {
        Err(store_error())
    }
    ...
    fn revoke(&mut self, _: &TokenDigest, _: u64) -> Result<()> {
        Err(store_error())
    }
    fn remove_expired(&mut self, _: u64) -> Result<u64> {
        Err(store_error())
    }
}

fn store_error() -> AuthError {
    AuthError::new(ErrorKind::Store, "store unavailable")
}

#[test]
fn token_store_failures_are_not_silently_treated_as_guests() {
    let tokens = TokenManager::new(FailingTokenStore {
        panic_on_get: false,
    });
    let user = Principal::new("user:1").unwrap();
    assert_eq!(
        tokens.issue(user, ["read"], 0).unwrap_err().kind(),
        ErrorKind::Store
    );
    let bearer = format!("bst_{}", "0".repeat(64));
    assert_eq!(
        tokens.authenticate(&bearer, 0).unwrap_err().kind(),
        ErrorKind::Store
    );
    assert_eq!(
        tokens
            .revoke(&TokenDigest::from_bytes([0; 32]), 0)
            .unwrap_err()
            .kind(),
        ErrorKind::Store
    );
...
impl SessionStore for PanickingSessionStore {
    fn put(&mut self, _: TokenDigest, _: SessionRecord) -> Result<()> {
        Ok(())
    }
    fn get(&mut self, _: &TokenDigest) -> Result<Option<SessionRecord>> {
        panic!("simulated store panic")
    }
    fn remove(&mut self, _: &TokenDigest) -> Result<()> {
        Ok(())
    }
    fn remove_expired(&mut self, _: u64) -> Result<u64> {
        Ok(0)
...
fn poisoned_session_store_also_fails_closed() {
    ...
    assert_eq!(
        sessions.authenticate(&bearer, 0).unwrap_err().kind(),
        ErrorKind::Store
    );
}
    ...
    let handles: Vec<_> = (0..8)
        .map(|index| {
            ...
            std::thread::spawn(move || {
                ...
                let token = tokens
                    .issue(Principal::new(&subject).unwrap(), ["read"], 0)
                    .unwrap();
                assert_eq!(
                    tokens
                        .authenticate(token.expose(), 0)
                        .unwrap()
                        .unwrap()
                        .subject(),
                    subject
