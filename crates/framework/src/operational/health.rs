...
impl HealthRegistry {
    ...
    pub fn readiness(&self) -> Result<Response> {
        ...
        let checks = snapshot
            ...
            .map(|check| {
                Json::Object(BTreeMap::from([
                    ...
                    (
                        "status".into(),
                        Json::from(if check.status == HealthStatus::Healthy {
                            "healthy"
                        } else {
                            "unhealthy"
                        }),
        ...
        let body = Json::Object(BTreeMap::from([
            (
                "status".into(),
                Json::from(if snapshot.ready { "ready" } else { "not_ready" }),
            ),
            ("checks".into(), Json::Array(checks)),
        ]));
        Ok(Response::json(&body)?.status(if snapshot.ready { 200 } else { 503 }))
    }
